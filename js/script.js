document.addEventListener('DOMContentLoaded', () => {
  ...
  const tableBody = document.getElementById('peliculasTable').querySelector('tbody');
  ...
  const addPelicula = async (pelicula) => {
      await fetch(`http://localhost:5000/nueva_pelicula`, {    
          method: 'POST',
          headers: {
  ...
  const updatePelicula = async (id, pelicula) => {
      await fetch(`http://localhost:5000/actualizar_pelicula/${id}`, {
          method: 'PUT',
          headers: {
              'Content-Type': 'application/json'
          },
  ...
  const deletePelicula = async (id) => {
      await fetch(`http://localhost:5000/eliminar_pelicula/${id}`, {
          method: 'DELETE'
      });
      fetchPeliculas();
  };
  ...
  form.addEventListener('submit', (e) => {
      ...
      const pelicula = { titulo, anioEstreno, categoriaId, duracion };
